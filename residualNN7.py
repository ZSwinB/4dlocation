...
# Data file path
filtered_data_path = r"D:\desktop\毕设材料\7\idealcoordinate7.xlsx"
...
receivers = np.array([
    ...
    [497,454,30],  # Receiver 7 position
])
...
def estimate_position(toa_values, receiver_indices, receiver_positions, label=None):
    ...
    selected_receivers = np.array([receiver_positions[i-1] for i in receiver_indices])  # Receiver IDs are 1-7
...
receiver_ids = df.iloc[:, 9:12].values.astype(int)  # three receiver IDs (1-7)
