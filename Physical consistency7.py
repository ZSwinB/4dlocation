...
excel_path = r"D:\desktop\毕设材料\7\output_classifier7.xlsx"
...
try:
   ...
   # df = pd.read_excel(excel_path, header=None, engine='openpyxl')
    ...
    print(df[['TOA1', 'TOA2', 'TOA3', 'TOA4', 'TOA5', 'TOA6','TOA7']].head())
    
    # Verify each column has enough variability (ensure values aren't all the same)
    for col in ['TOA1', 'TOA2', 'TOA3', 'TOA4', 'TOA5', 'TOA6','TOA7']:
...
try:
    ...
    if 'TOA1' in df.columns and df['TOA1'].nunique() < 10:
        ...
        print(df_xlrd[['TOA1', 'TOA2', 'TOA3', 'TOA4', 'TOA5', 'TOA6','TOA7']].head())
...
if 'TOA1' in df.columns:
    ...
    numeric_cols = ['x', 'y', 'TOA1', 'TOA2', 'TOA3', 'TOA4', 'TOA5', 'TOA6','TOA7']
    ...
    reflection_cols = ['label', 'TOA1_ray_type', 'TOA2_ray_type', 'TOA3_ray_type', 'TOA4_ray_type', 'TOA5_ray_type', 'TOA6_ray_type','TOA7_ray_type']
...
def predict_lowest_reflection_receivers(row, receiver_positions):
    ...
    all_combinations = list(combinations(range(7), 3))
...
def main_analysis():
    ...
    for index, row in df.iterrows():
        try:
            ...
            true_reflection_orders = np.array([
                ...
                int(row['TOA4_ray_type']), int(row['TOA5_ray_type']), int(row['TOA6_ray_type']), int(row['TOA7_ray_type'])
            ])
    ...
    # 保存为纯数据 Excel（无标题）
    output_path = r"D:\desktop\毕设材料\7\f_with_classifier7.xlsx"
