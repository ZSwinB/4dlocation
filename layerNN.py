...
toa_1 = data.iloc[:, 3].values  # first TOA
toa_2 = data.iloc[:, 4].values  # second TOA
toa_3 = data.iloc[:, 5].values  # third TOA
...
# Standardize TOA data
toa_data = np.column_stack((toa_1, toa_2, toa_3))
...
# Create model
input_size = X.shape[1]  # input feature dimension (3 TOAs + one-hot encoded area labels)
...
print('Model saved as: toa_model.pth')
...
"""
# Example new data
new_toa_data = np.array([[toa1_value, toa2_value, toa3_value]])
new_area_labels = np.array([area_label_value])

# Predict new location
predicted_locations = load_model_and_predict('toa_model.pth', new_toa_data, new_area_labels)
